...
1. Copy the `/ocap-web` folder somewhere on your computer. The `/ocap-web/static` folder's contents will be public.
1. Add a firewall rule and port-forwarding if necessary for `/ocap-web/ocap-webserver.exe`. The default port is 5000 and can be customized in `/ocap-web/options.json`.
1. Copy the `@ocap` subfolder, found in `/addons`, to the mod directory your Arma 3 server uses.
...
1. Copy the `/userconfig` folder to your Arma 3 directory. If a userconfig folder already exists, this will simply merge OCAP's configuration data. If not, this will create it.
...
1. Extract the contents to your `ocap-web/static/images/maps` folder.
...
*If someone tried to load a recording from a session played on a terrain that wasn't installed yet, the issue may persist even after installation. To fix this, they can clear their browser cache in order to force their system to re-download the terrain tiles from the server.*
